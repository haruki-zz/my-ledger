...
add column if not exists split_amount_a integer,
add column if not exists split_amount_b integer;

update public.recurring_expense_rules
set split_amount_a = round((amount_yen * split_ratio_a)::numeric / 100)::integer,
    split_amount_b = amount_yen - round((amount_yen * split_ratio_a)::numeric / 100)::integer
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'recurring_expense_rules_split_amounts_nonnegative_check'
      and conrelid = 'public.recurring_expense_rules'::regclass
  ) then
    alter table public.recurring_expense_rules
    add constraint recurring_expense_rules_split_amounts_nonnegative_check
    check (
      (split_amount_a is null or split_amount_a >= 0)
      and (split_amount_b is null or split_amount_b >= 0)
    );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'recurring_expense_rules_split_amounts_sum_check'
      and conrelid = 'public.recurring_expense_rules'::regclass
  ) then
    alter table public.recurring_expense_rules
    add constraint recurring_expense_rules_split_amounts_sum_check
    check (
      ownership <> 'shared'
      or (
        split_amount_a is not null
        and split_amount_b is not null
        and split_amount_a + split_amount_b = amount_yen
      )
    );
  end if;
end $$;

drop function if exists public.save_recurring_expense_rule_offline(
  uuid,
  uuid,
  text,
  text,
...
create function public.save_recurring_expense_rule_offline(
  p_rule_id uuid,
  p_ledger_id uuid,
  p_name text,
  p_category_id text,
