-- Fix legacy expenses mirror for recurring transactions whose source rule
-- exists only in recurring_transaction_rules.
--
-- Legacy public.expenses.recurring_rule_id still references
-- public.recurring_expense_rules(id). New v2 rules may not have a legacy row,
-- so the mirror must not write an invalid FK value.
